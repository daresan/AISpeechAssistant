...
class ImagineVideo:
	...
	def genVid(self, text, gen = None):
		...
		try:
			if len(text)>20:
				self.name = text[0:20].strip()
				self.name = self.name.split('.')
			else:
				self.name = text.split('.')
			self.name = self.name[0]
			print(self.name)
			self.name = "img_Video_"+self.name.replace(" ", "_")+"_"
			outpath = self.checkFilePath(self.name,0)
			print("Generating ", outpath)
			self.video_frames = self.pipe(text, num_inference_steps=interf, height=320, width=576, num_frames=36).frames[0]
	...
	def imagine(self, text, gen = None):
		if gen:
			return self.genVid(text, gen)
		else:
			return self.genVid(text)
		#if self.process:
		#	self.stop()
