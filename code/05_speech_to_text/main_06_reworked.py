		...
		try:
			while True:
				if self.check_mail:		
					...
					logger.debug("Checking mails for roy.batty.assistance@gmail.com with subject 'Simsalabim'")
					results = self.gmail.search_messages(self.service, "Simsalabim")
					print(f"Found {len(results)} results.")
					# for each email matched, read it (output plain/text to console & save HTML and attachments)
					for msg in results:
						self.gmail.read_message(self.service, msg)
					self.check_mail = False
					
				if self.send_mail:
					# test send email
					self.gmail.send_message(self.service, "daresan21@gmail.com", "Message from Roy Batty Assistant.", 
								"Hello Daniel! This is your AI Assistant Roy Batty. Have a nice day! Yours, Roy", ["TTS.py", self.attach])
					self.send_mail = False			
